...
import { contactLeadSchema } from '@lib/leads/schema';
...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  const result = await sendLead({ kind: 'contact', subject: 'New contact message — Microlines', lead: parsed.data });
  if (!result.ok) {
    console.error('Lead transport failed', result);
    return redirect('/contact?error=transport', 303);
  }
